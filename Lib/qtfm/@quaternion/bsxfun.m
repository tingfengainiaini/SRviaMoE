...
if strcmp(F, 'plus') || strcmp(F, 'minus')
    ...
    if ispure(A)
        C = quaternion(bsxfun(fun, A.x, B.x), ...
                       bsxfun(fun, A.y, B.y), ...
                       bsxfun(fun, A.z, B.z));
    else
...
% cases may be added as the need arises, or a more general method is found.
% Adding times to the supported list might permit the kron function to
% work, for example.

% $Id: bsxfun.m,v 1.5 2014/04/29 13:11:02 sangwine Exp $
