...
switch lower(moeModel.Experts.Type)
    case 'linear'
        %对于GatingsPosterior小于MinPosterior的情况，直接不考虑。如果此处用全部数据请替换为将这里所有的Index换为：
        ...
        if size(Target,2) == 1
            WeightTarget = (GatingPosterior(Index,:).^0.5).*Target;
            ...
            for j = 1:size(Target,2)
                ...
                hessianHidden = EInput'*EInput*InitBeta + moeModel.Experts.Lamda*eye(size(EInput,2));
                ExpertWeight = hessianHidden\(EInput'*WeightTarget*InitBeta + moeModel.Experts.Lamda*moeModel.Experts.WInit(:,j));
